...
OrionLauncherWindow::OrionLauncherWindow(QWidget *parent)
  ...
  connect(this, SIGNAL(signal_UpdatesListReceived(QList<CUpdateInfo>)), this,
          SLOT(slot_UpdatesListReceived(QList<CUpdateInfo>)));
  connect(this, SIGNAL(signal_BackupsListReceived(QList<CBackupInfo>)), this,
          SLOT(slot_BackupsListReceived(QList<CBackupInfo>)));
  connect(this, SIGNAL(signal_FileReceived(QByteArray, QString)), this,
          SLOT(slot_FileReceived(QByteArray, QString)));
  connect(this, SIGNAL(signal_FileReceivedNotification(QString)), this,
          SLOT(slot_FileReceivedNotification(QString)));
  connect(&m_UpdatesTimer, SIGNAL(timeout()), this,
          SLOT(slot_OnUpdatesTimer()));
  connect(&m_CheckClientCuoTimer, SIGNAL(timeout()), this,
          SLOT(slot_OnCheckClientCuoTimer()));
  ...
  CUpdateManager<OrionLauncherWindow>::GetFileInfo(qApp->applicationFilePath(),
                                                   version, crc32);
...
//----------------------------------------------------------------------------------
void OrionLauncherWindow::on_lw_ServerList_doubleClicked(
...
void OrionLauncherWindow::on_pb_ServerAdd_clicked() {
  ...
  for (int i = 0; i < ui->lw_ServerList->count(); i++) {
    ...
    if (item != nullptr && item->text().toLower() == name) {
      QMessageBox::critical(this, "Name is already exists",
                            "Server name is already exists!");
      return;
    }
  }

  CServerListItem *item = new CServerListItem(
      ui->le_ServerName->text(), ui->le_ServerAddress->text(),
      ui->le_ServerAccount->text(), ui->le_ServerPassword->text(),
      ui->le_ServeCharacter->text(), ui->cb_ServerEncryptPassword->isChecked());
...
void OrionLauncherWindow::on_pb_ServerSave_clicked() {
  ...
  CServerListItem *selected =
      (CServerListItem *)ui->lw_ServerList->currentItem();
...
void OrionLauncherWindow::on_pb_ProxyAdd_clicked() {
  ...
  if (!name.length()) {
    QMessageBox::critical(this, "Name is empty",
  ...
  for (int i = 0; i < ui->lw_ProxyList->count(); i++) {
    ...
    if (item != nullptr && item->text().toLower() == name) {
      QMessageBox::critical(this, "Name is already exists",
                            "Proxy server name is already exists!");
      return;
    }
  }

  ui->lw_ProxyList->addItem(new CProxyListItem(
      ui->le_ProxyName->text(), ui->le_ProxyAddress->text(),
      ui->le_ProxyPort->text(), ui->gb_ProxySocks5->isChecked(),
      ui->le_ProxyAccount->text(), ui->le_ProxyPassword->text(),
...
void OrionLauncherWindow::on_pb_ProxySave_clicked() {
  ...
  ui->cb_ServerProxy->setItemText(ui->lw_ProxyList->currentRow(),
...
void OrionLauncherWindow::SaveServerList() {
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    writter.writeAttribute("version", "0");
    writter.writeAttribute("size", QString::number(count));
    writter.writeAttribute("clientindex",
                           QString::number(ui->cb_OrionPath->currentIndex()));
    writter.writeAttribute(
        "closeafterlaunch",
        BoolToText(ui->cb_LaunchCloseAfterLaunch->isChecked()));
    writter.writeAttribute("lastserver",
                           QString::number(ui->lw_ServerList->currentRow()));
    writter.writeAttribute("checkupdates",
                           BoolToText(ui->cb_CheckUpdates->isChecked()));
    writter.writeAttribute("changeloglanguage",
                           ui->cb_ChangelogLanguage->currentText());
    writter.writeAttribute("noclientwarnings",
                           BoolToText(ui->cb_NoClientWarnings->isChecked()));
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (item != nullptr) {
        ...
        writter.writeAttribute("optionautologin",
                               BoolToText(item->GetOptionAutologin()));
        writter.writeAttribute("optionsavepassword",
                               BoolToText(item->GetOptionSavePassword()));
        writter.writeAttribute("optionsaveaero",
                               BoolToText(item->GetOptionSaveAero()));
        writter.writeAttribute("optionfastlogin",
                               BoolToText(item->GetOptionFastLogin()));
        writter.writeAttribute("optionrunuoam",
                               BoolToText(item->GetOptionRunUOAM()));
      ...
      if (reader.isStartElement()) {
        ...
        if (reader.name() == "proxylist") {
          ...
          if (attributes.hasAttribute("name")) {
            CProxyListItem *item =
                new CProxyListItem(attributes.value("name").toString());
            ...
            if (attributes.hasAttribute("socks5"))
              item->SetSocks5(
                  RawStringToBool(attributes.value("socks5").toString()));
...
void OrionLauncherWindow::LoadServerList() {
  ...
  if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    auto RemoveFileExtensionFunc = [](QString &str) {
      if (str.length() > 4 &&
          str.lastIndexOf(".exe", -4, Qt::CaseInsensitive) ==
              str.length() - 4) {
      ...
      if (reader.isStartElement()) {
        ...
        if (reader.name() == "serverlist") {
          ...
          if (attributes.hasAttribute("closeafterlaunch"))
            ui->cb_LaunchCloseAfterLaunch->setChecked(RawStringToBool(
                attributes.value("closeafterlaunch").toString()));
          ...
          if (attributes.hasAttribute("noclientwarnings"))
            ui->cb_NoClientWarnings->setChecked(RawStringToBool(
                attributes.value("noclientwarnings").toString()));
          ...
          if (attributes.hasAttribute("name")) {
            CServerListItem *item =
                new CServerListItem(attributes.value("name").toString());
            ...
            if (attributes.hasAttribute("optionautologin"))
              item->SetOptionAutologin(RawStringToBool(
                  attributes.value("optionautologin").toString()));

            if (attributes.hasAttribute("optionsavepassword"))
              item->SetOptionSavePassword(RawStringToBool(
                  attributes.value("optionsavepassword").toString()));

            if (attributes.hasAttribute("optionsaveaero"))
              item->SetOptionSaveAero(RawStringToBool(
                  attributes.value("optionsaveaero").toString()));

            if (attributes.hasAttribute("optionfastlogin"))
              item->SetOptionFastLogin(RawStringToBool(
                  attributes.value("optionfastlogin").toString()));

            if (attributes.hasAttribute("optionrunuoam"))
              item->SetOptionRunUOAM(RawStringToBool(
                  attributes.value("optionrunuoam").toString()));
...
void OrionLauncherWindow::on_tb_SetOrionPath_clicked() {
  ...
  QString path = QFileDialog::getExistingDirectory(
      nullptr, tr("Select OrionUO directory"), startPath);
...
//----------------------------------------------------------------------------------
QString OrionLauncherWindow::DecodeArgumentString(const char *text,
                                                  const int &length) {
  QString result = "";
...
//----------------------------------------------------------------------------------
QString OrionLauncherWindow::EncodeArgumentString(const char *text,
                                                  const int &length) {
  QString result = "";
...
//----------------------------------------------------------------------------------
void OrionLauncherWindow::RunProgram(const QString &exePath,
  ...
  if (CreateProcess(NULL, (LPWSTR)exePath.toStdWString().c_str(), NULL, NULL,
                    FALSE, 0, NULL, (LPWSTR)directory.toStdWString().c_str(),
                    &si, &pi)) {
...
void OrionLauncherWindow::on_pb_Launch_clicked() {
  ...
  if (!QFile::exists(ui->cb_OrionPath->currentText() + "/Client.cuo")) {
    QString configErrorText = "You miss file 'Client.cuo'!\nYou must create it "
                              "with ConfigurationEditor.exe tool.";
  ...
  command +=
      " -account:" +
      EncodeArgumentString(account.toStdString().c_str(), account.length()) +
      "," +
  ...
  if (character.length())
    command += " -autologinname:" +
               EncodeArgumentString(character.toStdString().c_str(),
                                    character.length());
  ...
  if (serverItem->GetUseProxy()) {
    ...
    for (int i = 0; i < ui->lw_ProxyList->count(); i++) {
      ...
      if (proxy != nullptr && proxy->text().toLower() == proxyName) {
        command +=
            " -proxyhost:" + proxy->GetAddress() + "," + proxy->GetProxyPort();

        if (proxy->GetSocks5()) {
          QString proxyAccount = proxy->GetAccount();
          QString proxyPassword = proxy->GetPassword();
          command += " -proxyaccount:" +
                     EncodeArgumentString(proxyAccount.toStdString().c_str(),
                                          proxyAccount.length()) +
                     "," +
                     EncodeArgumentString(proxyPassword.toStdString().c_str(),
...
//----------------------------------------------------------------------------------
void OrionLauncherWindow::on_lw_OAFeaturesOptions_clicked(
    const QModelIndex &index) {
  Q_UNUSED(index);
...
//----------------------------------------------------------------------------------
void OrionLauncherWindow::on_lw_OAFeaturesScripts_clicked(
    const QModelIndex &index) {
  Q_UNUSED(index);
...
void OrionLauncherWindow::slot_UpdatesListReceived(QList<CUpdateInfo> list) {
  ...
  for (const CUpdateInfo &info : list) {
    ...
    bool wantUpdate = !CUpdateManager<OrionLauncherWindow>::GetFileInfo(
        (info.UODir == "yes" ? directoryPath : qApp->applicationDirPath()) +
            "/" + info.Name,
        version, crc32);

    if (info.Version.length() &&
        CUpdateManager<OrionLauncherWindow>::TestVersions(version,
                                                          info.Version))
      wantUpdate = true;
...
void OrionLauncherWindow::slot_FileReceivedNotification(QString name) {
  ...
  if (m_FilesToUpdateCount <= 0 || ui->lw_AvailableUpdates->count() <= 0) {
    ...
    if (m_LauncherFoundInUpdates &&
        QFile::exists(qApp->applicationDirPath() + "/olupd.exe")) {
      SaveServerList();
      SaveProxyList();

      RunProgram(qApp->applicationDirPath() +
                     "/olupd.exe /OrionLauncher_Update.zip",
...
void OrionLauncherWindow::on_pb_CheckUpdates_clicked() {
  ...
  QtConcurrent::run(&CUpdateManager<OrionLauncherWindow>::CheckUpdates,
                    QStringList() << "www.orionuo.com"
                                  << "/Downloads/"
                                  << "OrionUpdate.html",
                    this);
}
...
void OrionLauncherWindow::on_pb_ApplyUpdates_clicked() {
  ...
  if (QMessageBox::question(this, "Updates notification",
                            "Close all OrionUO windows and press 'Yes'.\nPress "
  ...
  for (int i = 0; i < ui->lw_AvailableUpdates->count(); i++) {
    CUpdateInfoListWidgetItem *item =
        (CUpdateInfoListWidgetItem *)ui->lw_AvailableUpdates->item(i);
  ...
  if (m_FilesToUpdateCount) {
    for (CUpdateInfoListWidgetItem *item : updateList) {
      ...
      QtConcurrent::run(
          &CUpdateManager<OrionLauncherWindow>::DownloadFile,
          QStringList() << "www.orionuo.com"
                        << "/Downloads/" << item->m_Info.ZipFileName,
          this, QString(path + "/" + item->m_Info.ZipFileName), removeFile);
    }
  } else {
    ui->pb_CheckUpdates->setEnabled(true);
    ui->pb_ApplyUpdates->setEnabled(true);
...
void OrionLauncherWindow::on_pb_RestoreSelectedVersion_clicked() {
  CBackupInfoListWidgetItem *item =
      (CBackupInfoListWidgetItem *)ui->lw_Backups->currentItem();
  ...
  QtConcurrent::run(&CUpdateManager<OrionLauncherWindow>::DownloadFile,
                    QStringList()
                        << "www.orionuo.com"
                        << "/Downloads/" << item->m_Backup.ZipFileName,
                    this,
                    QString(ui->cb_OrionPath->currentText() + "/" +
                            item->m_Backup.ZipFileName),
                    true);

  QMessageBox::information(
      this, "Waiting for data...",
      "Download the backup/test version.\nClick OK and wait until the progress "
...
void OrionLauncherWindow::on_pb_ShowChangelog_clicked() {
  ...
  QtConcurrent::run(&CUpdateManager<ChangelogForm>::GetChangelog,
                    QStringList()
                        << "www.orionuo.com"
                        << "/Downloads/"
                        << ("OrionChangelog" +
                            ui->cb_ChangelogLanguage->currentText() + ".html"),
                    m_ChangelogForm);
}
//----------------------------------------------------------------------------------
void OrionLauncherWindow::on_lw_Backups_doubleClicked(
    const QModelIndex &index) {
  Q_UNUSED(index);
  on_pb_RestoreSelectedVersion_clicked();
}
//----------------------------------------------------------------------------------
void OrionLauncherWindow::on_lw_OrionFeaturesOptions_clicked(
    const QModelIndex &index) {
  Q_UNUSED(index);
  UpdateOrionFecturesCode();
}
